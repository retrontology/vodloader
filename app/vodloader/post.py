...
async def remove_original(self, video: VideoFile):
...
async def transcode(self, video: VideoFile):
    ...
    video.logger.info(f'Transcoding {self.path}')
    loop = asyncio.get_event_loop()
    video.transcode_path = await loop.run_in_executor(None, _transcode, video)
    await video.save()
    video.logger.info(f'Finished transcoding {self.path} to {self.transcode_path}')
    await remove_original(video)


def _transcode(self, video: VideoFile) -> Path:
